...
| Casos de Teste | Classes de Equivalência | Entradas | Resultado Esperado |
| :--- | :--- | :--- | :--- |
| **Caso 1** | 1, 4 | BPM: 60; Intervalo: 10s | Aceito (Normal, sem alerta) |
| **Caso 2** | 1, 4 | BPM: 80; Intervalo: 10s | Aceito (Normal, sem alerta) |
| **Caso 3** | 3, 4 | BPM: 101; Intervalo: 10s | Aceito e gera alerta de taquicardia |
| **Caso 4** | 3, 4 | BPM: 120; Intervalo: 10s | Aceito e gera alerta de taquicardia |
...
| --- | -------- | --------- | ------------------ |
| **Caso 1** | 1,4,7,10,13 | Método emoji válido, nota com 100 caracteres, primeiro registro do dia, horário padrão, usuário Premium com internet | Registro salvo com sucesso, normalizado para escala 1–10 e sincronizado |
| **Caso 2** | 2,4,7,10,13 | Nenhum método de registro selecionado | Sistema impede o registro e exibe mensagem de erro |
| **Caso 3** | 3,4,7,10,13 | Método de registro não suportado pelo sistema | Sistema rejeita o registro |
| **Caso 4** | 1,5,7,10,13 | Nota textual com mais de 500 caracteres | Sistema impede o salvamento e informa o limite permitido |
| **Caso 5** | 1,6,7,10,13 | Campo de nota contendo dado incompatível com texto | Sistema rejeita o registro |
| **Caso 6** | 1,4,8,10,13 | Usuário realiza novo registro na mesma data | Sistema informa a atualização e substitui o registro existente |
| **Caso 7** | 1,4,9,10,13 | Tentativa de criar um segundo registro no mesmo dia sem atualização | Sistema bloqueia a criação de um novo registro |
| **Caso 8** | 1,4,7,11,13 | Configuração de horário inexistente para notificação | Sistema rejeita a configuração |
| **Caso 9** | 1,4,7,12,13 | Configuração de horário em formato inválido | Sistema exibe erro de validação |
| **Caso 10** | 1,4,7,10,14 | Usuário Premium realiza registro sem conexão com a internet | Registro salvo localmente e sincronização adiada |
| **Caso 11** | 1,4,7,10,15 | Usuário Premium realiza registro com falha de sincronização causada por dados inconsistentes | Registro salvo localmente e sincronização rejeitada com registro de erro |
...
| Caso | Classes | Entradas | Resultado Esperado |
| ---- | -------- | --------- | ------------------ |
| **Caso 1** | 1,4,7,10,13 | Usuário com 5 registros na semana, perfil completo, plano Premium, filtro semanal e dados do aplicativo | Resumo semanal gerado com insights, sugestões e gráfico exibidos corretamente |
| **Caso 2** | 2,4,7,10,13 | Usuário com apenas 2 registros na semana | Sistema não gera o resumo e informa quantos registros ainda faltam |
| **Caso 3** | 3,4,7,10,13 | Usuário sem registros na semana | Sistema não gera o resumo e informa a ausência de registros suficientes |
| **Caso 4** | 1,5,7,10,13 | Usuário possui registros suficientes, mas não informou preferência de bem-estar | Sistema solicita o preenchimento da preferência antes de gerar sugestões |
| **Caso 5** | 1,6,7,10,13 | Usuário possui registros suficientes, mas não informou disponibilidade de tempo | Sistema solicita o preenchimento da disponibilidade antes de gerar sugestões |
| **Caso 6** | 1,4,8,10,13 | Usuário gratuito tenta acessar análise mensal comparativa ou histórico superior a 4 semanas | Sistema bloqueia o acesso e informa que o recurso é exclusivo para usuários Premium |
| **Caso 7** | 1,4,9,10,13 | Usuário tenta acessar insights avançados sem identificação válida do plano | Sistema impede o acesso aos insights avançados |
| **Caso 8** | 1,4,7,11,13 | Usuário seleciona um filtro inexistente para o gráfico | Sistema rejeita o filtro e exibe mensagem de erro |
| **Caso 9** | 1,4,7,12,13 | Usuário seleciona um filtro não suportado pelo sistema | Sistema rejeita a operação e solicita um filtro válido |
| **Caso 10** | 1,4,7,10,14 | Sistema recebe apenas dados provenientes de terceiros para geração dos insights | Sistema desconsidera os dados externos e não gera insights com essas informações |
| **Caso 11** | 1,4,7,10,15 | Sistema recebe dados do aplicativo misturados com fontes externas não permitidas | Sistema desconsidera os dados externos e gera os insights apenas com os dados válidos do aplicativo, ou bloqueia a geração caso não seja possível separar as fontes |
...
| **Caso 10** | 1,4,7,10,14 | Usuário autenticado acessa o próprio histórico; estilo "bíblico" selecionado; envia mensagem relatando abuso físico em curso; nenhum indicador adicional de crise aguda presente | Protocolo de encaminhamento acionado: fluxo interrompido, mensagem de acolhimento exibida, telefone CVV e profissional indicados; estilo substituído temporariamente pelo protocolo clínico; evento registrado com alerta no histórico |
| **Caso 11** | 1,4,7,10,15 | Usuário autenticado acessa o próprio histórico; estilo "psicológico" selecionado; envia manifestando crise de pânico em tempo real; nenhum indicador adicional de crise aguda presente | Protocolo de encaminhamento acionado: fluxo interrompido, mensagem de acolhimento exibida, telefone CVV e profissional indicados; estilo substituído temporariamente pelo protocolo clínico; evento registrado com alerta no histórico |
...
| **Caso 2** | 2,4,7,10,13 | Usuário free autenticado; seleciona filtro de 7 dias; período contém 4 registros com valores entre 1 e 7; exportação em PDF não solicitada | Relatório semanal gerado e exibido normalmente; funcionalidades exclusivas premium exibidas com bloqueio visual (cadeado) |
| **Caso 3** | 3,4,7,10,13 | Usuário não autenticado tenta acessar o relatório emocional com filtro de 7 dias e 4 registros no período | Acesso negado; sistema redireciona para tela de autenticação; relatório não é exibido |
| **Caso 4** | 1,5,7,10,13 | Usuário free autenticado; seleciona filtro de 14 dias; período contém 4 registros com valores entre 1 e 7; exportação em PDF não solicitada | Sistema exibe tela de convite para upgrade ao plano premium; conteúdo do relatório de 14 dias não é exibido |
| **Caso 5** | 1,6,7,10,13 | Usuário free autenticado; seleciona filtro de 30 dias; período contém 6 registros com valores entre 1 e 7; exportação em PDF não solicitada | Sistema exibe tela de convite para upgrade ao plano premium; conteúdo do relatório de 30 dias não é exibido |
...
| **Caso 7** | 1,4,9,10,13 | Usuário premium autenticado; seleciona filtro de 7 dias; nenhum registro emocional no período; solicita exportação em PDF | Relatório não é gerado; app exibe mensagem informando que não há registros no período selecionado; exportação em PDF indisponível |
...
| **Caso 10** | 1,4,7,10,14 | Usuário free autenticado; seleciona filtro de 7 dias; período contém 4 registros com valores entre 1 e 7; tenta exportar relatório em PDF | Exportação em PDF bloqueada; sistema exibe opção com cadeado e, ao ser acionada, apresenta tela de convite para upgrade ao plano premium |
| **Caso 11** | 1,4,7,10,15 | Usuário premium autenticado; seleciona filtro de 7 dias; nenhum registro emocional no período; tenta exportar relatório em PDF | Exportação em PDF indisponível; app exibe mensagem informando que não há registros no período selecionado para gerar o arquivo |
...
| Caso | Classes | Entradas | Resultado Esperado |
|---|---|---|---|
| **Caso 1** | 1,4,7,10,13 | Convite válido, aceito, permissão definida, mensagem de 50 caracteres, 3º apoiador | Vínculo criado e mensagem enviada com sucesso |
| **Caso 2** | **2**,4,7,10,13 | E-mail digitado de forma inválida no convite | Sistema rejeita o envio do convite |
| **Caso 3** | **3**,4,7,10,13 | Link de convite utilizado após 24 horas | Convite rejeitado por expiração |
| **Caso 4** | 1,**5**,7,10,13 | Convite enviado, convidado ainda não respondeu | Nenhum dado é compartilhado |
| **Caso 5** | 1,**6**,7,10,13 | Convite enviado, convidado recusa o vínculo | Vínculo não estabelecido |
| **Caso 6** | 1,4,**8**,10,13 | Vínculo aceito, mas sem nível de permissão definido | Sistema bloqueia o compartilhamento |
| **Caso 7** | 1,4,**9**,10,13 | Vínculo ativo, usuário remove o apoiador | Compartilhamento interrompido imediatamente |
| **Caso 8** | 1,4,7,**11**,13 | Vínculo ativo, mensagem de encorajamento vazia | Mensagem não enviada |
| **Caso 9** | 1,4,7,**12**,13 | Vínculo ativo, mensagem com 1010 caracteres | Mensagem rejeitada |
| **Caso 10** | 1,4,7,10,**14** | Usuário tenta vincular o 6º apoiador | Sistema rejeita (limite de 5 atingido) |
...
| Caso | Classes | Entradas | Resultado Esperado |
|---|---|---|---|
| **Caso 1** | 1,4,7,9,11,13 | Usuário autenticado conclui meta de emblema compatível com seu plano | Emblema desbloqueado com sucesso |
| **Caso 2** | **2**,4,7,9,11,13 | Usuário não autenticado tenta acessar emblemas | Operação negada |
| **Caso 3** | **3**,4,7,9,11,13 | Sessão expirada durante consulta de emblemas | Sistema solicita novo login |
| **Caso 4** | 1,**5**,7,9,11,13 | Meta ainda não concluída | Emblema não concedido |
| **Caso 5** | 1,**6**,7,9,11,13 | Meta marcada como inválida no sistema | Emblema não concedido, erro registrado |
| **Caso 6** | 1,4,**8**,9,11,13 | Emblema referenciado não existe na base de dados | Operação rejeitada (erro: não encontrado) |
| **Caso 7** | 1,4,7,**10**,11,13 | Emblema já conquistado anteriormente | Sistema não concede novamente |
| **Caso 8** | 1,4,7,9,**12**,13 | Emblema cadastrado sem nome ou descrição | Emblema não exibido, erro reportado |
| **Caso 9** | 1,4,7,9,11,**14** | Usuário gratuito tenta desbloquear emblema exclusivo premium | Cadeado exibido, meta oculta |
...
| Caso | Classes | Entradas | Resultado Esperado |
|---|---|---|---|
| **Caso 1** | 1,3,6,8,11 | Missão existente, primeira conclusão do dia, conta válida, XP chega a 100, dentro do limite | Pet evolui para o nível 2, 10 XP concedidos |
| **Caso 2** | **2**,3,6,8,11 | Usuário tenta concluir missão que não existe mais no sistema | Operação rejeitada |
| **Caso 3** | 1,**4**,6,8,11 | Missão já concluída hoje, tentativa de repetir | XP não concedido |
| **Caso 4** | 1,**5**,6,8,11 | Missão expirada após renovação diária | Missão não pode ser concluída |
| **Caso 5** | 1,3,**7**,8,11 | Usuário com conta inválida tenta concluir missão | Acesso negado |
| **Caso 6** | 1,3,6,**9**,11 | Usuário acumula 40 XP (abaixo do próximo marco de 100) | Pet não evolui, permanece no nível atual |
| **Caso 7** | 1,3,6,**10**,11 | Usuário acumula 520 XP (acima de 500) | Nenhuma evolução adicional, XP continua acumulando |
| **Caso 8** | 1,3,6,8,**12** | Usuário gratuito tenta concluir a 4ª missão do dia (já completou 3) | Sistema bloqueia, exibe aviso de limite diário atingido |
...
| Caso | Classes | Entradas | Resultado Esperado |
|---|---|---|---|
| **Caso 1** | 1,4,7 | Primeiro acesso, nome válido, estilo de orientação selecionado | Onboarding concluído com sucesso, usuário direcionado à tela principal |
| **Caso 2** | **2**,4,7 | Segundo acesso ao app | Onboarding não é exibido |
| **Caso 3** | **3**,4,7 | Usuário inválido tenta iniciar onboarding | Acesso negado |
| **Caso 4** | 1,**5**,7 | Primeiro acesso, nome do pet vazio | Finalização bloqueada |
| **Caso 5** | 1,**6**,7 | Primeiro acesso, nome do pet acima de 20 caracteres | Finalização bloqueada |
| **Caso 6** | 1,4,**8** | Primeiro acesso, nome válido, estilo de orientação não selecionado | Finalização bloqueada |
| **Caso 7** | 1,4,**9** | Primeiro acesso, nome válido, valor inválido enviado para o estilo de orientação | Sistema rejeita e solicita nova seleção |
...
| Caso | Classes | Entradas | Resultado Esperado |
|---|---|---|---|
| **Caso 1** | 1,4,7 | Permissão concedida, horário 23:00 válido, notificação ativada | Notificação enviada com sucesso |
| **Caso 2** | **2**,4,7 | Permissão negada pelo sistema operacional | Aviso exibido ao usuário, com atalho para configurações |
| **Caso 3** | **3**,4,7 | Permissão ainda não definida pelo usuário | Sistema solicita definição antes de configurar notificações |
| **Caso 4** | 1,**5**,7 | Permissão concedida, horário configurado como "25:80" | Configuração rejeitada |
| **Caso 5** | 1,**6**,7 | Permissão concedida, dois horários iguais cadastrados para o mesmo tipo de notificação | Cadastro rejeitado |
| **Caso 6** | 1,4,**8** | Permissão concedida, horário válido, notificação desativada pelo usuário | Nenhuma notificação enviada |
| **Caso 7** | 1,4,**9** | Permissão concedida, horário válido, configuração salva com dados inconsistentes | Sistema rejeita, mantém última configuração válida |
...
| Casos de Teste | Classes de Equivalência | Entradas | Resultado Esperado |
| :--- | :--- | :--- | :--- |
| **Caso 1** | 1, 4, 7 | E-mail: `user@gmail.com` ; Senha válida ; Código certo | Aceito e cadastro concluído |
| **Caso 2** | 2, 4, 7 | E-mail: `usergmail.com` | Rejeitado (Falta o @) |
| **Caso 3** | 1, 4, 7 | Senha: `Carlos123` | Aceita (Possui letras, números e tamanho correto) |
| **Caso 4** | 1, 5, 7 | Senha: `abcdefgh` | Rejeitada (Senha não possui números) |
| **caso 5** | 1, 4, 8 | Código recebido há 16 minutos | Rejeitado (Código passou da validade) |
