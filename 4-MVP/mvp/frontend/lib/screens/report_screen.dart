...
// ─── Dados mock: Meditação e BPM ────────────────────────────────────────────
//
// IMPORTANTE — mesma observação que fiz no termômetro emocional: hoje
// `meditation_history_screen.dart` e `monitor_screen.dart` guardam seus
// dados em listas fixas (`_mockHistory` / `_history`), sem persistir nada
// de verdade e sem timestamps reais (usam strings como 'Ontem, 22:15').
// Isso torna impossível filtrar por período de verdade.
//
// Pra o Relatório funcionar HOJE com filtro Semana/Mês, criei aqui dados de
// exemplo com `DateTime` real. Quando meditação e monitor de BPM passarem a
// persistir (ex: com um storage local nos mesmos moldes de
// `EmotionalEntryStorage`, ou via backend), é só trocar as duas funções
// `_mockMeditationSessions()` e `_mockBpmReadings()` abaixo por uma leitura
// real — o resto da tela (filtros, cálculos, cards) não precisa mudar.
