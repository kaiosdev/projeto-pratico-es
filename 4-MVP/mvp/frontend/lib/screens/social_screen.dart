...
// ─── Contexto importante sobre esta tela ───────────────────────────────────
//
// "Social" de verdade (perfis de outras pessoas, amigos, mensagens em tempo
// real) exige um backend com contas de usuário — hoje o projeto só tem
// `firebase_auth` (login), sem `cloud_firestore` nem nenhum servidor de
// mensagens. Então, assim como fiz no Chatbot, esta tela entrega a
// experiência de UI inteira (perfil, feed, mensagens) funcionando com dados
// locais/simulados, com comentários indicando exatamente onde plugar o
// backend real depois:
//   • Perfil → hoje salvo só no dispositivo (SharedPreferences). Pra virar
//     de verdade "social", precisa ir para o Firestore vinculado ao uid do
//     Firebase Auth.
//   • Feed ("compartilhar humor") → hoje só posts locais + 2 "amigos" mock.
//     Pra funcionar entre usuários reais, precisa de uma coleção
//     compartilhada (Firestore) ou endpoint no backend Node.js.
//   • Mensagens → hoje é uma conversa simulada por conversa (sem
//     WebSocket/real-time). Pra mensagens de verdade, o caminho comum é
...
class SocialFeedStorage {
  ...
  static Future<List<SocialPost>> load() async {
    ...
    // "Amigos" de exemplo, só pra o feed não ficar vazio antes de existir
...
class _ConversationScreenState extends State<_ConversationScreen> {
  ...
  void _send() {
    ...
    setState(() {
      ...
      // Resposta simulada, só pra ilustrar a UI — sem backend real-time.
