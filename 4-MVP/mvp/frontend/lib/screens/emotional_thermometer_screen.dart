...
// ─── Fonte de dados do termômetro emocional ─────────────────────────────────
//
// Usa o mesmo armazenamento compartilhado da tela de registro emocional
// (emotional_entry_storage.dart), na escala de 1 a 10. Enquanto não houver
// nenhum registro salvo (primeiro uso do app), exibe uma série de 30 dias
// de exemplo apenas para os gráficos não ficarem vazios; assim que o
// usuário salvar seu primeiro registro real, os dados de exemplo deixam
// de ser usados automaticamente.

typedef EmotionalEntry = record.EmotionalEntry;

/// Fornece a lista de registros para os gráficos do termômetro emocional,
/// com fallback para dados de exemplo enquanto não há registros reais.
class _EmotionalThermometerDataSource {
  static Future<List<EmotionalEntry>> load() async {
    final real = await record.EmotionalEntryStorage.loadAll();
    if (real.isNotEmpty) return real;

    // Série de exemplo (últimos 30 dias, escala 1-10) usada apenas até o
    // primeiro registro real existir.
...
class _EmotionalThermometerScreenState extends State<EmotionalThermometerScreen>
  ...
  Future<void> _loadData() async {
    final entries = await _EmotionalThermometerDataSource.load();
    setState(() {
  ...
  // Índice 0 fica sem uso (placeholder para "sem dados").
